...
struct async_looper __loopers[ASYNC_LOOPER_SIZE];

void async_looper_init(void) {
    int i;
    for (i = 0; i < ASYNC_LOOPER_SIZE; ++i) {
        INIT_LIST_HEAD(&__loopers[i].head);
        INIT_LIST_HEAD(&__loopers[i].timers);
        INIT_LIST_HEAD(&__loopers[i].event_calls);
        ringbuffer_init(&__loopers[i].rb, __loopers[i].buf, sizeof(__loopers[i].buf));
        list_add(&__loopers[i].head, &free_list);
    }
}

async_looper_t *__FAR async_looper_create(void) {
    async_looper_t *__FAR ret = ASYNC_LOOPER_CREATE_ERROR;
    async_lock_mutex(async_g_lock);
    if (list_empty(&free_list)) {
        async_unlock_mutex(async_g_lock);
        return ret;
    }
    ret = (async_looper_t *__FAR)(free_list.next);
    list_del(&ret->head);
    async_unlock_mutex(async_g_lock);
...
char async_looper_exit(struct async_looper *__FAR looper) {
...
void async_looper_loop(struct async_looper *__FAR looper) {
    unsigned char tmp;
