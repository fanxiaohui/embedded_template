...
static inline uint8_t is_leap_year(const struct datetime *datetime) {
...
static inline uint32_t calc_year(struct datetime *datetime, uint32_t second) {
    uint8_t i;
...
static inline uint32_t __yearToSeconds(const struct datetime *datetime) {
    return (datetime->year / 4) * SECONDS_PER_FOUR_YEARS + seconds_table_4year[datetime->year % 4];
}

static inline void calc_month_and_date(struct datetime *datetime, int dayOffsetOfYear) {
    uint8_t i;
...
static inline uint32_t __monthDateToSeconds(const struct datetime *datetime) {
    const uint16_t *table;
    table = is_leap_year(datetime) ? month_days_acc_table_leap_year : month_days_acc_table_com_year;
...
static inline void calc_time(struct datetime *datetime, uint32_t second) {
...
static inline uint32_t __timeToSeconds(const struct datetime *datetime) {
    return datetime->hour * SECONDS_PER_HOUR + datetime->minute * SECONDS_PER_MINUTE + datetime->second;
}

static uint8_t is_valid_datatime_ignore_weekday(const struct datetime *datetime) {
...
static uint32_t __toSeconds(const struct datetime *datetime) {
