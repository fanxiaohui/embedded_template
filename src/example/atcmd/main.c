...
static void uart_send(const uint8_t *buff, uint16_t size) {
    ...
    WriteFile(uart, buff, size, &dwSize, NULL);
}
...
int uart_recv(uint8_t *buffer, uint32_t size) {
    ...
    if (state.cbInQue > size) {
        ...
        ReadFile(uart, buffer, state.cbInQue, &dwRead, NULL);
    }
    return dwRead;
}

static const struct atcmd_interface atcmd_interface = {
    .init = uart_init,
    .send = uart_send,
};
...
int main(int argc, char **argv) {
    ...
    atcmd_init(&atcmd, recv_buff, sizeof(recv_buff), &atcmd_interface);
