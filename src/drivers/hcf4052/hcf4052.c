...
void hcf4052_init(const struct hcf4052_platform *__FAR platform) {
    (void)gpio_init(platform->gpio_ops, platform->ctrl_inh, GPIO_MODE_OUTPUT_PUSHPULL);
    (void)gpio_set_output(platform->gpio_ops, platform->ctrl_inh, 1);
    (void)gpio_init(platform->gpio_ops, platform->ctrl_a, GPIO_MODE_OUTPUT_PUSHPULL);
    (void)gpio_init(platform->gpio_ops, platform->ctrl_b, GPIO_MODE_OUTPUT_PUSHPULL);
}
