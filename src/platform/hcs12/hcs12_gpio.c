...
char hcs12_gpio_init(const struct hcs12_gpio *__FAR io, enum gpio_mode mode) {
...
char hcs12_gpio_input_is_high(const struct hcs12_gpio *__FAR io) {
...
char hcs12_gpio_output_is_high(const struct hcs12_gpio *__FAR io) {
    if (*io->dir_reg & io->bit) { // output mode
...
char hcs12_gpio_set_output(const struct hcs12_gpio *__FAR io, uint8_t is_high) {
    if (*io->dir_reg & io->bit) { // output mode
        if (is_high) {
            *io->out_reg |= io->bit;
        } else {
